 ...
 * (shared/src/*): a natural-language FAQ assistant, a structured
 * consistency check on the free-text adverse-event description, semantic
 * duplicate detection, and narrative document suggestions. None of this
...
export async function flagDescriptionInconsistencies(input: {
  ...
  const response = await client.messages.create({
    ...
    system:
      ...
      "description against the outcome checkboxes and recovery status the reporter selected, and " +
      "flag ONLY genuine inconsistencies or clearly missing information that would materially " +
      "affect how the report is reviewed — for example, the description mentions hospitalization " +
    ...
    messages: [
      {
        ...
        content:
          ...
          `Recovery status given: ${input.recoveryStatus || "(none given)"}\n\n` +
    ...
    tools: [
      {
        ...
        description:
          "Report inconsistencies found between the free-text description and the structured outcome/recovery fields.",
        strict: true,
        input_schema: {
          type: "object",
          properties: {
            issues: {
              ...
              items: {
                ...
                properties: {
                  field: {
                    type: "string",
                    enum: ["description", "outcomes", "recoveryStatus"],
                  },
                  issue: { type: "string" },
